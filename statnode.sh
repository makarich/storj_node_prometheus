...
printMetric "node_id{node_id=\"`echo $DASHDATA | jq -r .data.nodeID`\", version=\"`echo $DASHDATA | jq -r '(.data.version.major|tostring) + "." + (.data.version.minor|tostring) + "." + (.data.version.patch|tostring)'`\"}" "node_id" "gauge" 1 
